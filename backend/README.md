...
| 用途           | 対象者       | 推奨度   | エンドポイント                          |
| -------------- | ------------ | -------- | --------------------------------------- |
| お客さんの予約 | ゲスト       | ★★★ 推奨 | `POST /api/reservations/guest`          |
| 予約確認       | ゲスト       | ★★★ 推奨 | `GET /api/reservations/guest?email=xxx` |
| 予約管理       | スタッフ     | ★★★ 推奨 | `GET /api/reservations/current`         |
| 返却処理       | スタッフ     | ★★★ 推奨 | `POST /api/reservations/:id/return`     |
| 会員予約       | 認証ユーザー | △ 非推奨 | `POST /api/reservations`                |

**注意**: 認証ユーザー向けエンドポイントは将来的な会員制度のために残していますが、現時点では使用を推奨しない。
...
- **ゲスト予約**（推奨）: 認証不要。メールアドレスと名前のみで予約可能
- **認証ユーザー**（非推奨）: Supabase Auth を使用。将来的な会員制度用
